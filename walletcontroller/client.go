...
func (w *RPCWalletController) CreateTransactionWithInputs(
	...
	for _, reqInput := range requiredInputs {
		...
		for _, utxo := range utxos {
			if utxo.OutPoint.Hash.IsEqual(&reqInput.Hash) && utxo.OutPoint.Index == reqInput.Index {
				requiredUtxos = append(requiredUtxos, utxo)
				requiredValue += utxo.Amount
				found = true
				break
			}
		}
		if !found {
			return nil, fmt.Errorf("required input not found: %s:%d", reqInput.Hash, reqInput.Index)
		}
	}

	// Calculate total output value including fees
	totalOutput := btcutil.Amount(0)
	for _, output := range outputs {
		totalOutput += btcutil.Amount(output.Value)
	}

	// Estimate fee for transaction with required inputs
	estimatedSize := estimateTransactionSize(len(requiredUtxos), len(outputs), true)
	estimatedFee := btcutil.Amount(estimatedSize) * feeRatePerKb / 1000
	...
	if requiredValue < totalOutput+estimatedFee {
		...
		for _, utxo := range utxos {
			isRequired := false
			for _, reqUtxo := range requiredUtxos {
				if utxo.OutPoint.Hash.IsEqual(&reqUtxo.OutPoint.Hash) && utxo.OutPoint.Index == reqUtxo.OutPoint.Index {
					isRequired = true
					break
				}
			}
			if !isRequired && (additionalFilter == nil || additionalFilter(utxo)) {
				availableUtxos = append(availableUtxos, utxo)
			}
		}

		// Sort by amount (largest first)
		sort.Sort(sort.Reverse(byAmount(availableUtxos)))

		// Select additional UTXOs
		additionalValue := btcutil.Amount(0)
		for _, utxo := range availableUtxos {
			if additionalValue >= needed {
				break
			}
			additionalUtxos = append(additionalUtxos, utxo)
			additionalValue += utxo.Amount
		}
	...
	tx, err := buildTxFromOutputs(allUtxos, outputs, feeRatePerKb, changeScriptBytes)
	if err != nil {
		return nil, err
	}
...
func estimateTransactionSize(inputCount, outputCount int, includeWitness bool) int {
	// Basic transaction structure:
	// - Version: 4 bytes
	// - Input count: 1-9 bytes (varint)
	// - Inputs: ~148 bytes each (with witness data)
